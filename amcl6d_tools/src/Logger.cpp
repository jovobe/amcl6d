...
void Logger::printTimes(const int times, const std::string str,
...
// log a double* as a double**
bool Logger::log(double *d, const int rows, const int cols) const {
	if (m_output && d) {
		*m_stream << std::showpos << std::fixed << std::right;
		for (int i = 0; i < (rows * cols); ++i) {
			if ((i % cols == 0) && (i != 0)) {
				*m_stream << std::endl;
			}
			*m_stream << std::setprecision(3) << std::setw(10);
			try {
				*m_stream << d[i] << m_separator;
			} catch (...) {
				return false;
			}
		}
