...
{
	os << showpos << fixed << right;
	for (int i = 0; i < 16; i++)
	{
		if((i % 4 == 0)) os << std::endl;
		os << setprecision(3) << std::setw(10);
		os << matrix[i] << " ";
	}
	os << std::endl;
	return os;
 ...
 */
template <class T>
inline T Dist2(const T *x1, const T *x2)
{
  T dx = x2[0] - x1[0];
  T dy = x2[1] - x1[1];
  T dz = x2[2] - x1[2];
...
{
  return (int) ((float)rnd * std::rand() / (RAND_MAX + 1.0));
}
...
template <typename T>
static inline void M4copy(const T *Min, T *Mout)
{
	for(int i = 0; i < 16; i++)
	{
		Mout[i] = Min[i];
...
template <typename T>
static inline void M4create(T *M)
{
	M = new T[16];
	M4identity(M);
}
 ...
 * @param pose       vector containing the translation
 * @param matrix     matrix to be computed
 */
inline void AAToMatrix(double *aa, double *pose, double *matrix ){
